...
namespace EPYTST.Infrastructure.Data
{
    public class DapperCRUDService<T> : IDapperCRUDService<T> where T : class, IDapperBaseEntity
    {
        ...
        public async Task<dynamic> GetDynamicDataAsyncTest(string query)
        {
            try
            {
                await Connection.OpenAsync();
                ...
                else
                {
                    ...
                    foreach (var property in properties)
                    {
                        ...
                        if (propInfo == null || !propInfo.CanWrite)
                            continue;
                        var newValue = propInfo.GetValue(entity, null);
        ...
        public async Task<TType> GetScalarAsync<TType>(string query, object parameters = null)
        {
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DBConnection")))
            {
                await connection.OpenAsync();
                return await connection.ExecuteScalarAsync<TType>(query, parameters);
            }
        }
    }
}
